...
def build_shared_network(X, add_summaries=False):
    ...
    lstm_output, lstm_state = tf.nn.dynamic_rnn(
        cell=lstm_cell,
        inputs=fc1,
        initial_state=lstm_cell.zero_state(fc1.shape[0], tf.float32)
    )
...
class PolicyEstimator():
    ...
    def __init__(self, num_outputs, reuse=False, trainable=True):
        ...
        self.states = tf.placeholder(
            shape=[None, 64, 64, 3], dtype=tf.uint8, name="X")
        # The TD target value
        ...
        with tf.variable_scope("policy_net"):
            ...
            gather_indices = (
                tf.range(batch_size) * tf.shape(self.probs)[1] + self.actions)
...
class ValueEstimator():
    ...
    def __init__(self, reuse=False, trainable=True):
        # Placeholders for our input
        # Our input are 4 RGB frames of shape 160, 160 each
        self.states = tf.placeholder(shape=[None, 64, 64, 3], dtype=tf.uint8, name="X")
        # The TD target value
